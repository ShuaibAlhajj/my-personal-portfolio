...
## 2026-01-24 - Accessible Form Feedback Loops
**Learning:** Combining `.sr-only` labels with `aria-live="polite"` success containers allows for a micro-UX that is visually clean but fully accessible. Using `textContent` instead of `innerHTML` for these messages prevents XSS when reflecting user input (like names) back in the success message.
**Action:** Always implement a dedicated feedback element with `aria-live` for async form actions, and ensure all inputs have associated labels, even if visually hidden.
